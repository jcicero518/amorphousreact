...
/**
 * Handlebars.registerHelper('link', function(text, email, lrank, ...args) {
    const contactFormPath = '/contact-us';
    const pathName = location.pathname;
    const currTitle = document.title;
    email = Handlebars.Utils.escapeExpression(email);
    email = `${email}@sunycgcc.edu`;
    let instructorType = titleCase(lrank);
    let result = `<a href="${contactFormPath}?frompage=${currTitle}&instructoremail=${email}&instructor=${instructorType} ${args[0]} ${args[1]}">Email ${instructorType} ${text}</a>`;
